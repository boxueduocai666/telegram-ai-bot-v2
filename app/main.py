...
async def fetch_history_events(month: int, day: int, year: int | None, language: str = "en") -> list[dict[str, Any]]:
    ...
    events.sort(
        key=lambda item: (
            0 if item.get("year") is not None else 1,
            abs(int(item.get("year", 0)) - (year or date.today().year)),
        )
    )
    ...
    for event in events:
        key = (
            event.get("year"),
            event.get("text"),
            event.get("pages", [{}])[0].get("title") if event.get("pages") else "",
        )
        if key in seen:
            continue
        seen.add(key)
...
async def history_scheduler(state: BotState, stop_event: asyncio.Event) -> None:
    ...
    while not stop_event.is_set():
        try:
            ...
            for group_settings in state.db.list_auto_history_groups():
                try:
                    local_now = now_utc.astimezone(ZoneInfo(group_settings.timezone))
                    expected = group_settings.auto_history_time
                    current = local_now.strftime("%H:%M")
                    date_key = local_now.strftime("%Y-%m-%d")
                    if current != expected or group_settings.last_history_sent == date_key:
                        continue
                    ...
                    rendered = markdown_to_markdown_v2(
                        render_history_events(local_now.month, local_now.day, None, events)
                    )
                    try:
                        for chunk in split_text(rendered):
                            await state.application.bot.send_message(group_settings.chat_id, chunk, parse_mode="MarkdownV2")  # type: ignore[union-attr]
                        await state.db.mark_history_sent(group_settings.chat_id, date_key)
                    except Exception:
                        logger.exception("Automatic history send failed for chat %s", group_settings.chat_id)
                        try:
                            for chunk in split_text(render_history_events(local_now.month, local_now.day, None, events)):
                                await state.application.bot.send_message(group_settings.chat_id, chunk)  # type: ignore[union-attr]
                            await state.db.mark_history_sent(group_settings.chat_id, date_key)
                        except Exception:
                            logger.exception("Automatic history plain-text fallback failed for chat %s", group_settings.chat_id)
                except Exception:
                    logger.exception("Failed scheduled history for chat %s", group_settings.chat_id)
...
async def lifespan(app: FastAPI):
    ...
    db = Database(settings.database_url, settings.default_timezone, settings.default_auto_time)
    db.initialize()
    ...
    await bot_application.bot.set_my_commands(private_commands, scope=BotCommandScopeAllPrivateChats())
    await bot_application.bot.set_my_commands(group_commands, scope=BotCommandScopeAllGroupChats())
