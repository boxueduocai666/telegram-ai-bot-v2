...
async def history_scheduler(state: BotState, stop_event: asyncio.Event) -> None:
    ...
    while not stop_event.is_set():
        try:
            ...
            # Daily AI summaries use the same per-group timezone as /history.
            # The check runs every 30 seconds, so a group does not need any new
            # message at exactly 23:00 for the summary to be triggered.
            for settings in state.db.list_auto_summary_groups():
                try:
                    local_now = now_utc.astimezone(ZoneInfo(settings.timezone))
                    expected = settings.auto_summary_time
                    current = local_now.strftime("%H:%M")
                    date_key = local_now.strftime("%Y-%m-%d")
                    if current != expected or settings.last_summary_sent == date_key:
                        continue
                    await run_scheduled_group_summary(state, settings.chat_id, date_key)
                except Exception:
                    logger.exception("Failed scheduled group summary for chat %s", settings.chat_id)
