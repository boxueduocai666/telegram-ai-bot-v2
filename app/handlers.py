...
class BotState:
    ...
    def mark_bot_activity(self, chat_id: int, timestamp: datetime | None = None) -> None:
        self.last_bot_activity[chat_id] = timestamp or datetime.now(ZoneInfo("UTC"))
    ...
    def clear_group_history_ids(self, chat_id: int, message_ids: list[int]) -> None:
        bucket = self.group_histories.get(chat_id)
        if not bucket:
            return
        ...
        if not ids:
            return
        kept = [line for line in bucket if not line.message_id or line.message_id not in ids]
        bucket.clear()
        bucket.extend(kept)
        ...
        try:
            ...
            try:
                if index == 1:
                    ...
                    await update.effective_message.chat.send_message(fallback)
            except Exception:
...
async def _send_markdown_to_chat(chat: Any, text: str) -> None:
    ...
    for index, chunk in enumerate(rendered_chunks):
        try:
            await chat.send_message(chunk, parse_mode="MarkdownV2")
        except Exception:
            logger.exception("MarkdownV2 standalone send failed for chunk %s; falling back to plain text", index + 1)
...
async def run_scheduled_group_summary(
    ...
    """Generate and send one daily group summary.

    There is deliberately no 30-message trigger anymore. The scheduler decides
    when this function runs (normally 23:00 in the group's timezone). Messages
    are deleted only after the complete summary has been sent successfully.
    """
    if chat_id in state.summary_in_progress:
        return False
    state.summary_in_progress.add(chat_id)
    try:
        lines = await _load_summary_lines(state, chat_id)
        ...
        if message_ids:
            if not state.db.finalize_summary(chat_id, [int(message_id) for message_id in message_ids], date_key):
                logger.error(
                    "Scheduled summary sent but finalizing database state failed for chat %s",
                    chat_id,
                )
                return False
            state.clear_group_history_ids(chat_id, [int(message_id) for message_id in message_ids])
        elif not state.db.mark_summary_sent(chat_id, date_key):
            logger.error("Failed to mark scheduled summary as sent for chat %s", chat_id)
            return False

        state.mark_bot_activity(chat_id)
        logger.info("Scheduled group summary sent for chat %s", chat_id)
        return True
    except Exception:
...
async def summary_auto_command(
    update: Update,
    ...
    if token in {"on", "off"}:
        ...
        if state.db.save_group_settings(new):
            await update.effective_message.reply_text(
                f"✅ 每日自动群总结已{'开启' if enabled else '关闭'}。"
                f"时间：{new.auto_summary_time}（{new.timezone}）"
            )
        else:
...
async def summary_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        rendered = markdown_to_markdown_v2(result)
        ...
        try:
            ...
            logger.exception("Manual summary MarkdownV2 edit failed")
...
async def _send_ai_answer(update: Update, answer: str) -> None:
    ...
    rendered = markdown_to_markdown_v2(answer)
    chunks = split_text(rendered)
    if not chunks:
        await thinking.edit_text("（AI 返回了空消息）")
        return
    try:
        await thinking.edit_text(chunks[0], parse_mode="MarkdownV2")
    except Exception:
...
async def _record_group_message(update: Update, state: BotState) -> bool:
    ...
    # Touch the settings row so a newly seen group participates in the daily
    # 23:00 summary scheduler without any manual setup.
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        if should_handle:
            ...
            await _send_ai_answer(update, answer)
            if group_chat:
        ...
        # Automatic summaries are scheduled centrally at the configured daily time.
        # There is intentionally no 30-message trigger here.
        pass
