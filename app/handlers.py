...
async def _send_markdown_to_chat(chat: Any, text: str) -> None:
    try:
        ...
        logger.exception("MarkdownV2 standalone send failed; falling back to plain text")
        for chunk in split_text(text):
            await chat.send_message(chunk)


async def _run_automatic_group_summary(state: BotState, chat_id: int, user_id: int) -> None:
    if chat_id in state.summary_in_progress:
        return
    state.summary_in_progress.add(chat_id)
    try:
        rows = state.db.get_group_messages(chat_id, limit=120)
        if len(rows) < AUTO_SUMMARY_MESSAGE_COUNT:
            return
        lines: list[ChatLine] = []
        for row in rows:
            timestamp = None
        ...
        if last_message_id:
            state.db.clear_group_messages(chat_id, through_message_id=last_message_id)
            state.clear_group_history_through(chat_id, last_message_id)
    except Exception:
        logger.exception("Automatic group summary failed for chat %s", chat_id)
    finally:
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        if should_handle:
            ...
            if image_message is not None:
                ...
                if group_chat and previous_context:
                    # The current message has already been recorded; remove only its own last entry.
        ...
        if group_chat and state.db.count_group_messages(update.effective_chat.id) >= AUTO_SUMMARY_MESSAGE_COUNT:
            asyncio.create_task(
                _run_automatic_group_summary(
                    state,
