...
from .summary import ChatLine, summarize_chat_result, render_chat_summary
from .utils import (
    ReplyContext,
    build_reply_context,
...
class BotState:
    ...
    def add_group_message(
        ...
        """Add a normal group message to context and retain its Telegram jump link."""
...
async def summary_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        result = await summarize_chat_result(state.ai, chat_lines, model)
        rendered = result if isinstance(result, str) else render_chat_summary(result)
        await send_markdown(update, rendered)
    except Exception:
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.effective_chat.type != ChatType.PRIVATE:
        ...
        if raw_group_text and not raw_group_text.lower().lstrip().startswith("/"):
            ...
            # python-telegram-bot exposes Message.link for messages that have a
            # usable Telegram deep link (public groups and supergroups/channels).
            # If Telegram cannot provide one, the topic remains plain text.
            message_link = getattr(update.effective_message, "link", None)
        ...
        if len(state.group_histories[update.effective_chat.id]) >= AUTO_SUMMARY_MESSAGE_COUNT:
            try:
                ...
                auto_result = await summarize_chat_result(state.ai, recent, model)
                rendered_auto = (
                    auto_result
                    if isinstance(auto_result, str)
                    else render_chat_summary(auto_result)
                )
                await send_markdown(update, rendered_auto)
