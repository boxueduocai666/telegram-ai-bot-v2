...
class Settings:
    ...
    # Render Free has an ephemeral filesystem, so use a writable project-local
    # path by default. Users with a persistent disk can override DATABASE_PATH.
...
def load_settings() -> Settings:
    ...
    # PUBLIC_URL has highest priority for manual overrides. On Render,
    # RENDER_EXTERNAL_URL is provided automatically. Keep the Railway fallback
    # so the same code remains usable on the old deployment target if needed.
    public_url = _env(
        "PUBLIC_URL",
