...
def load_settings() -> Settings:
    ...
    # AVAILABLE_MODELS (first entry), then to the V2 default model.
    ...
    # Render reliably exposes RENDER_EXTERNAL_HOSTNAME for Web Services.
    # Prefer the explicit public URL, then Render's full URL, then its hostname.
    # Railway's public domain remains the final fallback for portability.
    public_url = _env(
        "PUBLIC_URL",
        _env(
            "RENDER_EXTERNAL_URL",
            _env("RENDER_EXTERNAL_HOSTNAME", _env("RAILWAY_PUBLIC_DOMAIN")),
        ),
    )
    ...
    # This watchdog is deliberately independent from FastAPI/PTB so that even
    # if the application's own startup hook fails to register the webhook, the
    # process can repair it without human intervention.
    try:
        from .webhook_watchdog import start_webhook_watchdog
        ...
        # Never prevent the bot from starting just because the watchdog could
        # not be initialized. The main application still has its normal webhook
        # registration path.
        import logging

        logging.getLogger(__name__).exception("Failed to start Telegram Webhook watchdog")
