...
请把提供的群聊整理成一份自然、好读、像“群聊日报/新闻速览”的总结。
最终内容要兼顾旧版 Telegram AI Bot 的可读性和新版 V2 的信息统计。
...
JSON 格式：
{
  "topics": [
    {
      "title": "简短自然的主题标题",
      "summary": "具体说明发生了什么、谁参与、事情如何发展；可以自然保留少量原话",
      "message_indexes": [1]
    }
  ],
  "overall_conclusion": "对本次群聊整体氛围和内容做简短总结；如果内容很少，也要诚实说明。"
}

要求：
- 只总结聊天里真实出现的内容，不编造人物、时间、结论、因果或态度。
- 自动忽略“哈哈”“6”“好的”“收到”、单独表情、纯打招呼等低信息量消息。
- 优先识别真正发生变化或形成讨论的内容：新闻、问题、争论、项目、产品、考试、活动、故障、计划、链接分享等。
- 对连续发送、风格相同或明显属于同一件事的消息进行合并，不要一句消息一个话题。
- 话题数量按实际内容决定；聊天少就少写，聊天多则完整覆盖主要内容。
- 每个话题通常写 2~4 句，让读者看得出发生了什么和讨论怎么发展。
- 人名可以直接使用消息里的显示名，不要擅自给人起外号。
- 可以适量保留有代表性的原话，让总结有群聊味道，但不要堆砌原文。
- 每个话题至少选择 1 条最有代表性的 message_indexes；编号必须真实存在。
- 一个话题可以选择多个 message_indexes，但优先选择 1~3 条最关键的。
- overall_conclusion 要像 V1 的“总体结论”，简洁说明活跃程度、主要参与者、主要内容和整体氛围。
- 不要输出 URL，不要输出“相关消息”区域；Bot 会根据 message_indexes 自动生成可点击标题链接。
- 不要在 JSON 里加入序号、emoji、Markdown 标记。
"""
